...
//				collector.addUpload((IUploadableInputInfo) info);
	...
	public void setInputParams(ASMWorkflow wkfInstance) throws InputSupplyException {
		for (InputInfo info : getInfos()) {
			...
			String oldparams = ASMService.getInstance().getCommandLineArg(wkfInstance.getUserID(),
			...
			LOGGER.debug("Altered input parameters for " + info.getJobName() + "\n\tFROM: " + oldparams + "\n\tTO: "
					+ params);
			ASMService.getInstance().setCommandLineArg(wkfInstance.getUserID(), wkfInstance.getWorkflowName(),
					job.getJobname(), params);
		}
	}

	// TODO: setting remote paths by adapter? used?
	public List<String> setOuputRemotes(ASMWorkflow wkfInstance, String basePath) {
		...
		for (ASMJob job : wkfInstance.getJobs().values()) {
			...
			for (Entry<String, String> port : job.getOutput_ports().entrySet()) {
				...
				if (ASMService.getInstance().getRemoteOutputPath(wkfInstance.getUserID(),
						wkfInstance.getWorkflowName(), job.getJobname(), port.getKey()) != null
						|| !ASMService
								.getInstance()
								.getRemoteOutputPath(wkfInstance.getUserID(), wkfInstance.getWorkflowName(),
					...
					ASMService.getInstance().setRemoteInputPath(wkfInstance.getUserID(), wkfInstance.getWorkflowName(),
