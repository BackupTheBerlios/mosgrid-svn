 ...
 * 
 * CAUTION!!! Modified default listener to use standart cast instead of
 * reflection to enable derivates of vaadin Window class. Backwards
 * compatibility is lost this way.
 * 
 * @author Sami Ekblad
 * 
 */
		...
		Button.ClickListener cb = new Button.ClickListener() {
			...
			public void buttonClick(ClickEvent event) {
				...
				if (confirm.isEnabled()) {
					...
					// } catch (Exception e) {
					// throw new
					// RuntimeException("Failed to remove confirmation dialof from the parent window.",
					// e);
					// }
					// }
					// WORKAROUND END
					...
					Component parent = confirm.getParent();
					if (parent instanceof Window) {
						((Window) parent).removeWindow(confirm);
					}
