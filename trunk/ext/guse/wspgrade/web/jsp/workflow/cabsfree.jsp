...
<form method="post" action="${pURL}">
	...
	<tr>
	    <td width="25%" class="khead"><div class="bold"><msg:getText key="text.ctemplate.newtempname" /></div></td>
	    <td width="75%" class="kback"><c:out value="${newWorkflow}" escapeXml="true" /></td>
	</tr>
	<tr>
	    <td width="25%" class="head"><div class="bold"><msg:getText key="text.ctemplate.parentname" /></div></td>
...
<c:forEach var="tmp" items="${jobs}" varStatus="p">
    <tr><td class="khead"><msg:getText key="text.ctemplate.check" /></td><td class="khead">${tmp.name}</td><td class="khead"><c:out value="${tmp.txt}" escapeXml="true" /></td></tr>
	...
	<c:forEach var="plabel" items="${tmp.label}">
	    <c:if test="${plabel.key==tmp0.key}">
		<c:set var="label" value="${plabel.value}" />
                <c:if test="${plabel.value==''}">
                    <c:set var="pp" value="1" />
                </c:if>                
	    </c:if>	
	</c:forEach>   
	<c:forEach var="ppdesc" items="${tmp.desc0}">
	    <c:if test="${ppdesc.key==tmp0.key}">
		<c:set var="desc" value="${ppdesc.value}" />
	    </c:if>	
	</c:forEach> 
	<c:forEach var="pinherited" items="${tmp.inherited}">
	    <c:if test="${pinherited.key==tmp0.key}">
		<c:set var="inherited" value="${pinherited.value}" />
	    </c:if>	
	</c:forEach>         
	<c:choose>
	    <c:when test="${pp=='2'}">
                <c:set var="display" value="block" />
                <c:set var="dfree" value="checked" />
                <c:set var="dclose" value="" />
            </c:when>
            <c:otherwise>
                <c:set var="display" value="none" />
                <c:set var="dfree" value="" />
                <c:set var="dclose" value="checked" />
            </c:otherwise>
	</c:choose>
        
        <td class="${color}">
            <div style="display:none"><input type="checkbox" disabled="true" checked="true" name="job${p.index}_${tmp0.key}" id="job${p.index}_${tmp0.key}" value="${tmp0.value}"></div>
            <input type="radio" ${dclose} name="job${p.index}_${tmp0.key}" value="${tmp0.value}" onclick="document.getElementById('job${p.index}_${tmp0.key}').checked=true;document.getElementById('tmpdivjob${p.index}_${tmp0.key}').style.display='none';labelValueChecker('label_job${p.index}_${tmp0.key}',0);"><msg:getText key="text.ctemplate.close" />
            <input type="radio" ${dfree} name="job${p.index}_${tmp0.key}" value="${tmp0.value}" onclick="document.getElementById('job${p.index}_${tmp0.key}').checked=false;document.getElementById('tmpdivjob${p.index}_${tmp0.key}').style.display='block';labelValueChecker('label_job${p.index}_${tmp0.key}',1);"><msg:getText key="text.ctemplate.free" />
        </td>                 
    ...
    <tr>
	<td colspan="3">
    	    <div style="padding-left:20px;display:${display}" id="tmpdivjob${p.index}_${tmp0.key}">
		<table>
		    <tr>
			...
			<td>
				...
				<c:forEach var="tmpj" items="${jobs}">
                                    <c:choose>
                                        <c:when test="${tmpj.name==inherited}">
                                            <option selected>${tmpj.name}</option>
                                        </c:when>
                                        <c:otherwise>
			...
			<td><textarea cols="50" id="label_job${p.index}_${tmp0.key}" name="label_job${p.index}_${tmp0.key}"><c:out value="${label}" escapeXml="true" /></textarea></td>
			<td>
			...
			<td><textarea cols="50" rows="5" name="desc_job${p.index}_${tmp0.key}"><c:out value="${desc}" escapeXml="true" /></textarea></td>
			<td>
	...
	<tr><td class="khead"><msg:getText key="text.ctemplate.input" /></td><td class="khead"><c:out value="${tmp0.name}" escapeXml="true" /></td><td class="khead"><c:out value="${tmp0.txt}" escapeXml="true" /></td></tr>
	<c:forEach var="tmp1" items="${tmp0.data}" varStatus="ln">
		...
		<c:forEach var="ptmpl" items="${tmp0.dataDisabled}">
			...
			<c:set var="pp" value="1" />
                ...
                <c:forEach var="plabel" items="${tmp0.label}">
                    <c:if test="${plabel.key==tmp1.key}">
                        <c:set var="label" value="${plabel.value}" />
                        <c:if test="${plabel.value==''}">
                            <c:set var="pp" value="1" />
                        </c:if>
                ...
                <c:forEach var="ppdesc" items="${tmp0.desc}">
                    <c:if test="${ppdesc.key==tmp1.key}">
                        <c:set var="desc" value="${ppdesc.value}" />
                    </c:if>	
                </c:forEach> 
                <c:forEach var="pinherited" items="${tmp0.inherited}">
                    <c:if test="${pinherited.key==tmp1.key}">
                        <c:set var="inherited" value="${pinherited.value}" />
                    </c:if>	
                </c:forEach>
 ...
 <%-- enabled/disabled --%>
                ...
                <td class="${color}">																	                                          	
                    <input type="radio" value="${tmp1.value}" ${dclose} name="job${p.index}_input${tmp0.id}_${tmp1.key}" value="${tmp1.value}" onclick="javascript:document.getElementById('tmpdivinput${p.index}_${tmp0.name}_${tmp1.key}').style.display='none';labelValueChecker('label_job${p.index}_input${tmp0.id}_${tmp1.key}',0);"><msg:getText key="text.ctemplate.close" />
                    <input type="radio" value="${tmp1.value}" ${dfree} name="job${p.index}_input${tmp0.id}_${tmp1.key}" onclick="document.getElementById('tmpdivinput${p.index}_${tmp0.name}_${tmp1.key}').style.display='block';labelValueChecker('label_job${p.index}_input${tmp0.id}_${tmp1.key}',1);"><msg:getText key="text.ctemplate.free" />
                </td>
	    ...
	    <tr>
		<td colspan="3">                  
		    <div id="tmpdivinput${p.index}_${tmp0.name}_${tmp1.key}" style="display:${display}">
			...
			<table>
				...
				<td>
					...
					<c:forEach var="tmpj" items="${jobs}">
					    <c:forEach var="tmpj0" items="${tmpj.inputs}" varStatus="i">
                                                <c:set var="ih" value="${tmpj.name}/${tmpj0.name}" />
                                                <c:choose>
                                                    <c:when test="${ih==inherited}">
                                                        <option selected>${ih}</option>
                                                    </c:when>
                                                    <c:otherwise>
                                                        <option>${ih}</option>
                                                    </c:otherwise>                                        
			    ...
			    <tr>
				<td><msg:getText key="text.ctemplate.label" /></td>
				<td><textarea cols="50"  id="label_job${p.index}_input${tmp0.id}_${tmp1.key}" name="label_job${p.index}_input${tmp0.id}_${tmp1.key}"><c:out value="${label}" escapeXml="true" /></textarea></td>
				<td>
			    ...
			    <tr>
				<td><msg:getText key="text.ctemplate.description" /></td>
				<td><textarea cols="50" name="desc_job${p.index}_input${tmp0.id}_${tmp1.key}" rows="5"><c:out value="${desc}" escapeXml="true" /></textarea></td>
				<td>
 ...
 <%-- enabled/disabled --%>
                ...
                <td class="${color}">
                    <input type="radio" value="${tmp1.value}" ${dclose} name="job${p.index}_output${tmp0.id}_${tmp1.key}" value="${tmp1.value}" onclick="javascript:document.getElementById('tmpdivoutput${p.index}_${tmp0.name}_${tmp1.key}').style.display='none';labelValueChecker('label_job${p.index}_output${tmp0.id}_${tmp1.key}',0);"><msg:getText key="text.ctemplate.close" />
                    <input type="radio" value="${tmp1.value}" ${dfree} name="job${p.index}_output${tmp0.id}_${tmp1.key}" onclick="document.getElementById('tmpdivoutput${p.index}_${tmp0.name}_${tmp1.key}').style.display='block';labelValueChecker('label_job${p.index}_output${tmp0.id}_${tmp1.key}',1);"><msg:getText key="text.ctemplate.free" />
                </td>
	    ...
	    <tr>
		<td colspan="3">
		    <div id="tmpdivoutput${p.index}_${tmp0.name}_${tmp1.key}" style="display:${display}">
			...
			<table>
			    ...
			    <tr>
				<td><msg:getText key="text.ctemplate.label" /></td>
				<td><textarea cols="50" id="label_job${p.index}_output${tmp0.id}_${tmp1.key}" name="label_job${p.index}_output${tmp0.id}_${tmp1.key}"><c:out value="${label}" escapeXml="true" /></textarea></td>
				<td>
			    ...
			    <tr>
				<td><msg:getText key="text.ctemplate.description" /></td>
				<td><textarea cols="50" rows="5" name="desc_job${p.index}_output${tmp0.id}_${tmp1.key}"><c:out value="${desc}" escapeXml="true" /></textarea></td>
				<td>
