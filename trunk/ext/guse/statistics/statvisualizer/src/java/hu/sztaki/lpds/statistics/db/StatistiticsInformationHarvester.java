...
public class StatistiticsInformationHarvester extends InformationHarvester{
    ...
    public void getWorkflowInstance(Map<String, List<MetricInformation>> metrics, String wrtid) {
        ...
        try {
            ...
            //Get stats from stat_WorkflowInstance JOIN stat_statistics
            ps = con.prepareStatement("SELECT *  "
                                      + " FROM `stat_statistics` JOIN `stat_WorkflowInstance` ON "
                                      + "`stat_statistics`.`ID` = `stat_WorkflowInstance`.`statistics_ID`"
                                      + "WHERE wrtid = ?");
    ...
    public void getConcreteWorkflow(Map<String, List<MetricInformation>> metrics, String wfid) {
        ...
        try {
            con = connectionSource.getConnection();
            //Get statistics from ConcreteWorkflow
            ps = con.prepareStatement("SELECT * "
                                      + " FROM `stat_statistics` JOIN `stat_ConcreteWorkflow` ON "
                                      + "`stat_statistics`.`ID` = `stat_ConcreteWorkflow`.`statistics_ID`"
                                      + "WHERE wfid = ?");
    ...
    public void getAbstractJob(Map<String, List<MetricInformation>> metrics, String jobName, String wfId) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT *  "
                                      + " FROM `stat_statistics` JOIN `stat_AbstractJob` ON "
                                      + "`stat_statistics`.`ID` = `stat_AbstractJob`.`statistics_ID`"
                                      + "WHERE jobName = ? AND wfid = ?");
    ...
    public void getUser(Map<String, List<MetricInformation>> metrics, String userID) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT *  "
                                      + " FROM `stat_statistics` JOIN `stat_user` ON "
                                      + "`stat_statistics`.`ID` = `stat_user`.`statistics_ID`"
                                      + "WHERE userID = ?");
    ...
    public void getPortal(Map<String, List<MetricInformation>> metrics, String URL) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT * "
                                      + " FROM `stat_statistics` JOIN `stat_portal` ON "
                                      + "`stat_statistics`.`ID` = `stat_portal`.`statistics_ID`"
                                      + "WHERE URL = ?");
            ps.setString(1, URL);
            populateMetricData(
                    ps, metrics, con);
    ...
    public void getResource(Map<String, List<MetricInformation>> metrics, String URL) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT *  "
                                      + " FROM `stat_statistics` JOIN `stat_resource` ON "
                                      + "`stat_statistics`.`ID` = `stat_resource`.`statistics_ID`"
                                      + "WHERE URL = ?");
            ps.setString(1, URL);
            populateMetricData(
                    ps, metrics, con);
    ...
    public void getDCI(Map<String, List<MetricInformation>> metrics, String name) {
        ...
        try {
            ...
            ps = con.prepareStatement("SELECT *  "
                                      + " FROM `stat_statistics` JOIN `stat_DCI` ON "
                                      + "`stat_statistics`.`ID` = `stat_DCI`.`statistics_ID`"
                                      + "WHERE name = ?");
            ps.setString(1, name);
            populateMetricData(
                    ps, metrics, con);
    ...
    private void populateMetricData(PreparedStatement ps, Map<String, List<MetricInformation>> metrics, Connection con) {
        ...
        try {
            ...
            if (rst.next()) {
                ...
                rst.close();
                ps.close();
                //Get statistics about the state the job progresses through
