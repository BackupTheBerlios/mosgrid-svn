...
{
    /** Creates a new instance of WfsWfiServerImpl */
    ...
    {
// not in use
//        if(!"false".equals(PropertyLoader.getInstance().getProperty("guse.wfs.system.savestatus")))
//        new SaveStatusThread(pData);
   ...
   public String getResourceType(JobStatusBean pData)
   {
        ...
        try
        {
            ...
            while(rst.next()){gridID=rst.getString(1);}
            conn.close();
        }
        catch(Exception e){e.printStackTrace();}
///            if(gridID.equals(""))
//                gridID="test";
   ...
   public HashMap getCollectionResourceType(ResourceCollectionBean pData)
   {
       ...
       try
       {
           ...
           ResultSet rst =stmt.executeQuery(sql);
           while(rst.next())
               res.put(rst.getString(1),rst.getString(2));
           ...
           if(rst.getInt(1)>0)
           {
               ...
               while(rst.next())
               {
                   ...
                   {           
                       if(res.get(rst.getString(1))==null)
                            res.put(rst.getString(1),rst.getString(2));
                   }
                   else
                        res.put(rst.getString(1),rst.getString(2));
...
//           
    ...
    {
        try{return Base.getI().getJobDescriptionFromCache(pValue.getPortalID(), pValue.getUserID(), pValue.getWorkflowID(), pValue.getWorkflowRuntimeID(), pValue.getJobID());}
...
//            System.out.println("---"+sql);
            ResultSet rst =stmt.executeQuery(sql);
            while(rst.next())
//            {res.put("job."+new String(rst.getString(1)), new String(rst.getString(2)));}
            {
                if(!rst.getString(1).startsWith("tmp*"))
                res.append("\t<job-property "+rst.getString(1)+"=\""+repl(rst.getString(2))+"\" />\n");
            }
//output
...
//                    System.out.println("-Chanel input-"+ssql);
                    ...
                    while(rst1.next())
                    {itt++;
                        if(!rst1.getString(2).equals("intname"))
                        res.append("\t<input-property name=\""+rst0.getString(3)+"\" "+rst1.getString(2)+"=\"\">"+repl(rst1.getString(3))+"</input-property>\n");
                    }
                    res.append("\t<input-property name=\""+rst0.getString(3)+"\" seq=\"\">"+rst0.getString(4)+"</input-property>\n");
                    if(itt==0)
                        res.append("\t<input-property name=\""+rst0.getString(3)+"\" fake=\"fake\"></input-property>\n");
                    tps.close();
            }
            conn0.close();
//configured data
