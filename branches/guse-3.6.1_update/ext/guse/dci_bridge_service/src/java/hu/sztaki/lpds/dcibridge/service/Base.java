...
public class Base extends Thread{
	...
	public static final String[] dcis=new String[]{MIDDLEWARE_GLITE,MIDDLEWARE_GT2,MIDDLEWARE_GT4,
            MIDDLEWARE_UNICORE,MIDDLEWARE_ARC,MIDDLEWARE_LSF,MIDDLEWARE_PBS,MIDDLEWARE_SERVICE,
            MIDDLEWARE_BOINC,MIDDLEWARE_GEMLCA,MIDDLEWARE_GAE,MIDDLEWARE_LOCAL,MIDDLEWARE_GBAC, MIDDLEWARE_EDGI};//, MIDDLEWARE_CLOUDBROKER
...
//init loggspool
            ...
            for(dci.data.Middleware t:Conf.getM().getMiddleware())
                initMiddleware(t);
    }
    ...
    public void initMiddleware(dci.data.Middleware pMiddleware){
        ...
        if(pMiddleware.isEnabled()){
            if(Base.getI().getMiddlewares(pMiddleware.getType())!=null) {
                ...
                try{Base.getI().getMiddlewares(pMiddleware.getType()).get(0).setConfiguration();}
                catch(Exception e){writeLogg(Base.getI().getLogg(pMiddleware.getType()),new LB(e));}
            }
            for(int i=activeThreadCount;i<pMiddleware.getThreads();i++){
                try{createNewMiddlewarePlugininstance(pMiddleware.getType(), pMiddleware.getPlugin());}
                catch(Exception e){writeLogg(loggs.get(MIDDLEWARE_SYSTEM), new LB(e));}
            }
 ...
 */
    public void createNewMiddlewarePlugininstance(String pType, String pClassName) throws Exception{
        if(middlewares.get(pType)==null)
            middlewares.put(pType, new java.util.concurrent.CopyOnWriteArrayList<Middleware>());
        Middleware tmp=(Middleware)Class.forName(pClassName).newInstance();
        if(middlewares.get(pType).size()==0){
            try{tmp.setConfiguration();}
            catch(Exception e){writeLogg(Base.getI().getLogg(pType),new LB(e));}
        }
 ...
 */
    public void initWorkDir(){
        ...
        if(Conf.getS().getPath()==null);
        else if("".equals(Conf.getS().getPath()));
        else path=Conf.getS().getPath();
 ...
 */
    public Middleware getMiddleware(String pValue) {
        ...
        for(byte b=0;b<middlewares.get(pValue).size();b++)
            if(tmp.get(b).isState() && tmp.get(b).getSize()<min){
 ...
 */
    public List<Middleware> getMiddlewares(String pValue) {return middlewares.get(pValue);}
 ...
 */
    public synchronized void setPath(String pValue){path=pValue;}
 ...
 */
    ...
    public boolean isDestroyedJob(String pID){return jobs.get(pID)==null;}
/**
    ...
    public void newJob(String pID,Job pValue){
        ...
        try{dwlThread.addJob(pValue);}
            ...
            pValue.setStatus(ActivityStateEnumeration.FAILED);
                /*@ToDo standard error output is missing.........................................*/
            }
    }
 ...
 */
    public DownloadingThread getDWLThread(){return dwlThread;}
/**
...
/*@ToDo standard output is missing.........................................*/
        }
    }
 ...
 */
    public void removeJob(String pID){
        if(0==Conf.getP().getDebug())
            deleteWorkDir(getJob(pID));
        jobs.remove(pID);}
/**
 ...
 */
    ...
    public static synchronized  void writeLogg(List<LB> pLogList,LB pLogg){
        ...
        pLogList.add(pLogg);
        System.out.println(pLogg.getInfo());
    }

    public static void writeLogg(String pLogListName,LB pLogg){writeLogg(getI().loggs.get(pLogListName),pLogg);}

    public List<LB> getLogg(String pName){
        if(loggs.get(pName)==null)
            loggs.put(pName, new CopyOnWriteArrayList<LB>());
    ...
    public static void endPluginLogg(String pMiddleware,String pKey, short pLevel){
        LB tmp;
        if(jobloggs.get(pKey)==null) tmp=new LB();
        else tmp=jobloggs.remove(pKey);
    ...
    public static void writeJobLogg(String pKey, short pLevel, String pMSG){
        LB tmp;
        if(jobloggs.get(pKey)==null) tmp=new LB(pMSG);
        else tmp=jobloggs.remove(pKey);
        tmp.setTs1(System.currentTimeMillis());
        try {
        ...
        catch (IOException ex) {writeLogg(getI().loggs.get(MIDDLEWARE_SYSTEM), new LB(ex));}
    }
    ...
    public static void writeJobLogg(Job pJob, Exception e, String pMessage){
        try {
            ...
            fw.write("\t\t"+t.getClassName()+"."+t.getMethodName()+"("+t.getFileName()+":"+t.getLineNumber()+")"+"\n");
            fw.write("\t</info>\n");
            fw.write("</item>\n");
            fw.close();
        }
