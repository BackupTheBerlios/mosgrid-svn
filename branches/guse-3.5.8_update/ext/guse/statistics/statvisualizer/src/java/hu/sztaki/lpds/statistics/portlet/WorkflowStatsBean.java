...
/*
        ...
        */
        ...
        /*
        ms = mif.getMetric(StatisticLevel.CONCRETEWORKFLOW);        //Gets metric information for concrete workflow
        statsFac.getConcreteWorkflow(ms, workflowId);               //Gets statistics for Concrete Workflows
        */
        
        this.workflowName = mp.getWFIDs(this.userId).get(workflowId);


        workflowInstances = mp.getWRTIDs(workflowId);               //Gets Map for workflow Instances
        
        jobNames = mp.getAJobs(workflowId);                         //Gets list for abstract jobs
        /*
