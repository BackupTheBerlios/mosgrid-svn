...
public class Grid_boinc extends Middleware{
    ...
    protected final static int maxJob=2000;
...
//Job in Status queue
    ...
    @Override
    protected void submit(Job pJob) throws Exception {throw new UnsupportedOperationException("Not supported yet.");}

    @Override
    protected void getOutputs(Job pJob) throws Exception {throw new UnsupportedOperationException("Not supported yet.");}

    @Override
    protected void getStatus(Job pJob) throws Exception {throw new UnsupportedOperationException("Not supported yet.");}
        ...
        while(true){
            ...
            while(enm.hasMoreElements()){
                ...
                if(Conf.getItem(THIS_MIDDLEWARE, bridgeID).isEnabled()){
                    ...
                    try{
                        if(bridge.getSubmitQueue().size()>0) {submit(bridge);event=true;}
                        ...
                        catch(Exception e0){Base.writeLogg(THIS_MIDDLEWARE,new LB(e0));}
                    }
                }
            }
            if(!event){
                Base.writeLogg(THIS_MIDDLEWARE,new LB("sleep:"+LAST_ACTIVATE_TIMESTAMP));
                try{sleep(LAST_ACTIVATE_TIMESTAMP);}
                catch(InterruptedException e){Base.writeLogg(THIS_MIDDLEWARE,new LB(e));}
            }
        }
    }
            ...
            while(pClient.getSubmitQueue().size()>0 &&count<maxJob){
                ...
                if(item.getFlag()!=Middleware.ABORT){
                   ...
                   try{
                       ...
                       for(LogicalFile bt:boincjob.getInputs())
                           Base.writeJobLogg(item.getId(), LB.INFO, "input:"+bt.getLogicalName()+"=>"+bt.getURL());
                       for(String bt:boincjob.getOutputs())
                           Base.writeJobLogg(item.getId(), LB.INFO, "output:"+bt);
    ...
    private void download(BridgeHandler pClient){
        ...
        for(int i=0;i<outputs.getOutput().size();i++){
            ...
            try{
                ...
                e0.printStackTrace();
                try{Base.getI().getJob(guseID).setStatus(ActivityStateEnumeration.FAILED);}
                catch(Exception e1){/*job mar nincs*/}        
            }
