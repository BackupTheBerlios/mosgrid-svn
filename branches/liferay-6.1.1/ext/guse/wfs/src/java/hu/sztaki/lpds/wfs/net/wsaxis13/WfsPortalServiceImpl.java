...
public class WfsPortalServiceImpl implements WfsPortalService
{
    ...
    public Vector getAbstractWorkflows(ComDataBean value)
    {
        ...
        try
        {
            ...
            // old sql="SELECT name,txt FROM aworkflow WHERE id_portal='"+value.getPortalID()+"' and id_user='"+value.getUserID()+"'";
//            System.out.println(">>>>>"+sql);            
    ...
    {
        Vector res=new Vector();
//        StatusHandler sth=new StatusHandler();
        try
        {
            Connection conn = Base.getConnection();
            Statement stmt = conn.createStatement();
            if ((value.getWorkflowID() == null) || ("".equals(value.getWorkflowID())) || ("null".equals(value.getWorkflowID()))) {
                // workflow lista lekerdezese
            ...
            // old sql="SELECT b.name,b.txt,a.txt,b.id,a.name FROM aworkflow as a,workflow as b WHERE id_portal='"+value.getPortalID()+"' and id_user='"+value.getUserID()+"' and id_aworkflow=a.id and wtyp=-1 ORDER BY b.id";
//            System.out.println(sql);
            ResultSet rst =stmt.executeQuery(sql);
            ...
            {
                tmp=new ComDataBean();
                // tmp.setTxt((rst.getString(2)!="")?rst.getString(2):rst.getString(3));
...
//            System.out.println(">>>>>"+sql);
            ...
            while(rst.next())
            {
                tmp=new ComDataBean();
                String ttmp="--";
                // old if (rst.getLong(5)==(-1)) {
                //    sql="SELECT a.name FROM workflow as a WHERE wtyp="+rst.getLong(5);
                //    ResultSet rst0 =stmt0.executeQuery(sql);
                //    ttmp=rst0.getString(1);
                // old }
                if (rst.getLong(5) > 0) 
                {
...
//                System.out.println(">>>>>"+sql);
                ...
                while(rst0.next())
                {
                    ...
                    if(rst0.getString(2).equals("status"))
                    {
                                ...
                                if(Status.isFinished(ti)||Status.isError(ti))
                                    tmp.setStatus(rst0.getString(3));
                                else
                                    tmp.setStatus(""+Status.ABORTED);
                    }
...
//                        System.out.println("---"+sql);
                        ...
                        while(rsti.next()){v.add(new PortBean("",rsti.getString(1), rsti.getString(2), "input",rsti.getString(3),rsti.getString(4)));}
//outputs
...
//                        System.out.println("---"+sql);
                        ...
                        while(rsto.next()){v.add(new PortBean(rsto.getString(3),rsto.getString(1), rsto.getString(2), "output"));}
...
//status resource
                        ...
                        while(rsts.next())
                        {
                            if(rsts.getString(1).equals("resource")){tmpJobData.setResource(rsts.getString(2));}
                            if(rsts.getString(1).equals("status"))
                            {
                                int ti=rsts.getInt(2);
                                if((ti==6)||(ti==7)||(ti==21)||(ti==25))
                                    tmpJobData.setStatus(rsts.getInt(2));
                                else
                                    tmpJobData.setStatus(15);
                            }
                        }
                        res.add(tmpJobData);
                    }
...
//last element
    ...
    {
//System.out.println("saveNewWorkflow()... getTyp() : " + value.getTyp() + " getWorkflowID() : " + value.getWorkflowID());
        // meaning of getTyp :
        // 0 concrete workflow from graph
        // 1 concrete workflow from abstract
        // 2 abdstract workflow from concrete
        // 3 the parent graph after abdstract workflow upload 
        // 4 concrete workflow from concrete 
        try
        {
            Connection conn = Base.getConnection();
            Statement stmt = conn.createStatement();
            ...
            if(value.getTyp().intValue()==0)
            {
                ...
                // System.out.println("saveNewWorkflow() == 0 type sql 1 : " + sql);
                ResultSet rst =stmt.executeQuery(sql);
                while(rst.next()){aWorkflowID=rst.getLong(1);}
                sql="INSERT INTO workflow(id_aworkflow,name,txt,wtyp) VALUES("+aWorkflowID+",'"+value.getWorkflowID()+"','"+value.getTxt()+"',0)";                       
                // System.out.println("saveNewWorkflow() == 0 type sql 2 : " + sql);                
                stmt.executeUpdate(sql);       
            }
            ...
            {
                long tid=0; //template id
                long wid=0; //source workflow id
...
//controlling of workflow configuration (saving to error_prop) - at concrete worflow configuration only.
            int wfType = value.getTyp().intValue();
            if (( wfType == 0) || ( wfType == 1) || ( wfType == 4)) {
                // note : ez a hivas nagyon sql eroforras igenyes a
                // methodusban levo getWorkflowConfigData() hivas miatt...
    ...
    public Boolean deleteWorkflowGraf(ComDataBean value) {
        try {
            // delete graf workflow
            Connection conn = Base.getConnection();
            long aworkflowID = 0;
            // Statement stmt = conn.createStatement();
            // String sql = "SELECT a.id FROM aworkflow as a WHERE a.name='" + value.getWorkflowID() +
            // "' and a.id_user='" + value.getUserID() + "' and a.id_portal='" + value.getPortalID() + "'";
            ...
            // delete graf data from ainput table
            ...
            // delete graf data from aoutput table
            ...
            //
            // delete graf data from ajob table
            sql = "DELETE FROM ajob WHERE id_aworkflow=?";
            preps = conn.prepareStatement(sql);
            preps.setLong(1, aworkflowID);
            preps.executeUpdate();
            //
            // delete graf data from aworkflow table
    ...
    public Boolean deleteWorkflow(ComDataBean value)
    {
        ...
        try
        {
            ...
            // System.out.println("+++"+value.getWorkflowRuntimeID());
            if(
                    (value.getWorkflowRuntimeID()==null)||
                    (value.getWorkflowRuntimeID().equals(""))||
                    (value.getWorkflowRuntimeID().endsWith("null"))
              )
            {
                // minden a workflowhoz tartozo workflow runtime adatait toroljuk...
                //
                // delete old workflow data from job_outputs table...
                ...
                while (rst.next()) {
                    ...
                    // System.out.println("wfs delete workflow delete job_outputs data...: " + wrtid);
                    wrtidHash.put(wrtid, "");
                }
                // System.out.println("wfs delete workflow delete wrtidhash : " + wrtidHash);
                if (!wrtidHash.isEmpty()) {
                    ...
                    while (wrtidEnum.hasMoreElements()) {
                        ...
                        // System.out.println("wfs delete workflow real delete job_outputs data...: " + wrtid);
...
//template
            ...
            {
                // Delete the given concrete workflow runtime data, only.
    ...
    public void submitWorkflow(ComDataBean value)
    {
        try
        {
            ...
            ResultSet rst =stmt.executeQuery(sql);
            while(rst.next()){workflowID=rst.getLong(1);}       
            sql="DELETE FROM status WHERE id_workflow="+workflowID;
            stmt.executeUpdate(sql);
            sql="DELETE FROM job_prop WHERE id_workflow="+workflowID;
            stmt.executeUpdate(sql);
...
//                System.out.println("->"+sql0);
                ...
                while(rst0.next())
                {
                    if(rst0.getString(1).equals("resource")){tmpJobData.setResource(rst0.getString(2));}
                    else if(rst0.getString(1).equals("status")){tmpJobData.setStatus(rst0.getInt(2));}
                }
                res.add(tmpJobData);
            }
            conn.close();
 ...
 */
    private OutputDescrioptionBean actualOutputPortWithEmbededAndGeneratorProperties(long pWFID, OutputDescrioptionBean pOutputPort) throws ClassNotFoundException, SQLException
    {
        ...
        ResultSet rs=ps.executeQuery();
        while(rs.next()){
            if("maincount".equals(rs.getString(1)))
                if(1<rs.getLong(2)) pOutputPort.setGenerator(true);
/*            if("ioutput".equals(rs.getString(1))){
                String[] values=rs.getString(2).split("/");
                pOutputPort.setRealjob(values[0]);
                pOutputPort.setRealport(values[1]);
            }*/
        }
        conn.close();
        return pOutputPort;
...
//get result
            for(JobAndOutputsBean jobOutputs:workflowjobsoutputs){
                ...
                while(enm.hasMoreElements()){
                    outputName=enm.nextElement();
//                    if(!jobOutputs.getOutputDescription(outputName).isGenerator())
    ...
    public void test(ComDataBean p){System.out.println(p);}
    ...
    public Vector getWorkflowConfigDataErrorReal(ComDataBean value) throws Exception {
        ...
        Connection conn = Base.getConnection();
        try {
            String esql = "SELECT ep.id_workflow, ep.jobname, ep.id_port, ep.id_error " +
                    "FROM aworkflow as aw, workflow as w, error_prop as ep " +
                    "WHERE aw.id = w.id_aworkflow and aw.id_portal = ? and aw.id_user = ? " +
                    "and w.name = ? and w.id = ep.id_workflow ORDER BY ep.jobname, ep.id_port";
            // System.out.println("params: " + value.getPortalID() + ", " + value.getUserID() + ", " + value.getWorkflowID());
    ...
    public void workflowConfigDataErrorCheck(ComDataBean value) {
        try {
            // csak konkret workflow-t kell ellenorizni
            if ((value.getTyp() == 0) || (value.getTyp() == 1) || (value.getTyp() == 4)) {
                Connection conn = Base.getConnection();
                String esql = "SELECT w.id, w.name, w.txt FROM aworkflow as aw, workflow as w WHERE " +
    ...
    public Boolean setWorkflowConfigDataReal(ComDataBean pID, Vector pData) throws Exception {
        // System.out.println("setWorkflowConfigDataReal()...");
        Vector res = pData;
        //
        // System.out.println("WFS INHERITED");
        for (int i = 0; i < res.size(); i++) {
            JobPropertyBean t = (JobPropertyBean) res.get(i);
            // System.out.println("++WFS-job:++" + t.getName());
            Iterator it = t.getInherited().keySet().iterator();
            while (it.hasNext()) {
                Object key = it.next();
                String inhvalue = "" + t.getInherited().get(key);
                if (!"---".equals(inhvalue) && !"null".equals(inhvalue)) {//inherited
                    // System.out.println("job::" + key + "=" + inhvalue);
            ...
            try {
                for (int ii = 0; ii < t.getInputs().size(); ii++) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (!"---".equals(inhvalue) && !"null".equals(inhvalue)) {//inherited
                            // System.out.println("InPUT::" + ((PortDataBean) t.getInputs().get(ii)).getName() + "::" + key + "=" + inhvalue);
                            String[] jp = inhvalue.split("/");// "job/port"
                            for (int j = 0; j < res.size(); j++) {//job seek
                                if (jp[0].equals(((JobPropertyBean) res.get(j)).getName())) {
                                    for (int jjp = 0; jjp < ((JobPropertyBean) res.get(j)).getInputs().size(); jjp++) {//jobport seek
            ...
            try {
                for (int ii = 0; ii < t.getOutputs().size(); ii++) {
                    ...
                    while (it.hasNext()) {
                        ...
                        if (!"---".equals(inhvalue) && !"null".equals(inhvalue)) {//inherited
                            // System.out.println("OutPUT::" + ((PortDataBean) t.getOutputs().get(ii)).getName() + "::" + key + "=" + inhvalue);
                            String[] jp = inhvalue.split("/");// "job/port"
                            for (int j = 0; j < res.size(); j++) {//job seek
                                if (jp[0].equals(((JobPropertyBean) res.get(j)).getName())) {
                                    for (int jjp = 0; jjp < ((JobPropertyBean) res.get(j)).getOutputs().size(); jjp++) {//jobport seek
...
//logg
        ...
        for(int i=0; i<res.size();i++) {
            ...
            while(it.hasNext()) {
                Object key=it.next();
                System.out.println("-"+key+":"+jpb.getExe().get(key)+":"+jpb.getLabel().get(key));
            }
            for(int j=0;j<jpb.getInputs().size();j++) {
                PortDataBean pd=(PortDataBean)jpb.getInputs().get(j);
                it=pd.getData().keySet().iterator();
                System.out.println("********INPUT("+j+"/"+jpb.getInputs().size()+")*****");
                System.out.println("--input--");
                while(it.hasNext()) {
                    Object key=it.next();
                    System.out.println("-"+key+":"+pd.getData().get(key)+":"+pd.getLabel().get(key));
                }
            }
        }
        */
        ...
        try {
            ...
            if (workflowID.equals("")) {
                ...
                saveNewWorkflow(pID);
                //
                sql = "SELECT a.id FROM workflow as a,aworkflow as b WHERE b.id_portal='" + pID.getPortalID() + "' and b.id_user='" +
                        pID.getUserID() + "' and a.name='" + pID.getWorkflowID() + "' and a.id_aworkflow=b.id";
                // System.out.println(sql);
            ...
            //
            // method invocation only in case of real worflow (in help: pID.setTyp() comment)
            //
...
//            System.out.println(sql);
            try
            {
                ...
                Statement stmt = conn.createStatement();
                stmt.executeUpdate(sql);
                // delete old desc (JDL/RSL key/values)
...
//                System.out.println(sql);
                ...
                {
                    String key=""+itl.next();
//                    System.out.println("INSERT INTO job_prop VALUES("+workflowID+","+tmp.getId()+",'"+key+"','"+tmp.getExe().get(key)+"','"+tmp.getLabel().get(key)+"','"+tmp.getDesc0().get(key)+"','"+tmp.getInherited().get(key)+"')");
...
//                    System.out.println(sql);
                    ...
                    {
                        String key=""+itl.next();
                        // sql="INSERT INTO input_prop VALUES("+workflowID+","+tmp0.getId()+",'"+key+"','"+tmp0.getData().get(key)+"')";
//                        System.out.println(sql);
//                        stmt.executeUpdate(sql);                        
//                        System.out.println("INSERT INTO input_prop (id_workflow, id_ainput, name, value,label,desc) VALUES('"+workflowID+"','"+tmp0.getId()+"','"+key+"','"+(String) tmp0.getData().get(key)+"','"+ tmp0.getLabel().get(key)+"','"+ tmp0.getDesc().get(key)+"')");        
                        prep.setString(1, workflowID);
                ...
                {
                    // PortDataBean tmp0 =(PortDataBean)tmp.getOutputs().get(j);
...
//                    System.out.println(sql);
                    ...
                    {
                        String key=""+itl.next();
                        if(key.equals("maincount"))maincountflag=true;  //uj
                        if(key.equals("intname"))intnameflag=true;  //uj
//                        sql="INSERT INTO output_prop VALUES("+workflowID+","+tmp0.getId()+",'"+key+"','"+tmp0.getData().get(key)+"','"+tmp0.getLabel().get(key)+"','"+tmp0.getDesc().get(key)+"','"+tmp0.getInherited().get(key)+"')";
                        prep.setString(1, workflowID);
                        prep.setLong(2, tmp0.getId());
                        prep.setString(3, key);
                        prep.setString(4, (String) tmp0.getData().get(key));
...
//            System.out.println("WFS:getWorkflowProps (size):" + res.size());
        } catch (Exception e) {
            e.printStackTrace();
        }
        try {
    ...
    public Vector getWorkflowConfigDataReal(ComDataBean value) throws Exception {
        // System.out.println("getWorkflowConfigDataReal... getWorkflowID : " + value.getWorkflowID());
        Vector res = new Vector();
        Connection conn = Base.getConnection();
        try {
            String workflowID = "0";
            String parentWorkflowID = "0";
            // get keys: workflowID, parentWorkflowID
            ...
            // System.out.println("workflowID : " + workflowID + ", parentWorkflowID : " + parentWorkflowID);
            //
            JobPropertyBean tmp;
...
//                        System.out.println("---"+ttmp.getInputs().get(ii).getClass());
//                        System.out.println("---"+((PortDataBean)ttmp.getInputs().get(ii)).getName());
...
//                        System.out.println("---"+ttmp.getOutputs().get(ii).getClass());
//                        System.out.println("---"+((PortDataBean)ttmp.getOutputs().get(ii)).getName());
                        enm = ((PortDataBean) ttmp.getOutputs().get(ii)).getData().keySet().iterator();
                        while (enm.hasNext()) {
                            key = "" + enm.next();
//                            System.out.println("RESULT:"+key+"->"+((PortDataBean)ttmp.getOutputs().get(ii)).getInherited().get(key));
    ...
    private JobPropertyBean getJobProperty(Connection pconn, String workflowID, String ajobID) throws Exception {
        ...
        try {
            ...
            while (rst.next()) {
                ...
                preps0.setString(2, rst.getString(1));// ainput.id
            ...
            while (rst.next()) {
                ...
                preps0.setString(2, rst.getString(1)); // aoutput.id
                ResultSet rst0 = preps0.executeQuery();
                while (rst0.next()) {
                    tmp0.put(rst0.getString(1), rst0.getString(2));
                    tmp1.put(rst0.getString(1), rst0.getString(3));
    ...
    public Vector getEmbedWorkflowConfigDataReal(ComDataBean value) throws Exception {
        ...
        try {
            ...
            while (rst.next()) {
                tmp = new JobPropertyBean();
                // significant change, it is faster
    ...
    private JobPropertyBean getEmbedJobProperty(Connection pconn, String workflowID, String ajobID) throws Exception {
        JobPropertyBean res = new JobPropertyBean();
        try {
            // job binaris adatai (a workflow beagyazasi adatok itt vannak letarolva iworkflow kulcs ertekkel)
    ...
    {
        // System.out.println("getWorkflowInstanceDesc()... " + pValue);
    ...
    public Vector getWorkflowInstanceJobs(ComDataBean value)
    {
        ...
        {
            long lindex = 0;
            // System.out.println("wfs getWorkflowInstanceJobs getSize : " + value.getSize());
            if (value.getSize() != null) {
                if (value.getSize() >= 0) {
                    lindex = value.getSize();
                }
            }
            // System.out.println("wfs getWorkflowInstanceJobs lindex : " + lindex);
            // the max size of query is set in portal side where the modification settings is happening
            int getmax = 2500;
            ...
            // System.out.println("wfs getWorkflowInstanceJobs sql : " + sql);
            ResultSet rst = stmt.executeQuery(sql);
            while(rst.next())
            {
...
//queries the old graph id; queries the wf id
            ...
            while (rst.next()) {
                ...
                // System.out.println("-----------old graf id:" + oldgrafid);
            }
//queries the old graph jobs
            ...
            while (rst.next()) {
                ...
                while (ojobnames.hasMoreElements()) {
                    ...
                    if (ojname.equals(jobname)) {
                        // System.out.println("### ---new jobid found, update job_props ---> new jobname:" + jobname + " id:" + id_ajob);
...
//input ports copy
                        ...
                        // System.out.println("### all oldinputs:" + oldinputs.keySet().toString());

                        //input ports of new graph (job)
                        sql = "SELECT * FROM ainput as ai WHERE ai.id_ajob='" + id_ajob + "' ";
                        rst2 = stmt2.executeQuery(sql);
                        while (rst2.next()) {//uj graf(job) input portjai - masolas
                            ...
                            if (op != null && ( ("".equals(prejob) && "".equals(preoutput) && "".equals(op.getPrejob()) && "".equals(op.getPreoutput())) || (!"".equals(prejob) && !"".equals(preoutput) && !"".equals(op.getPrejob()) && !"".equals(op.getPreoutput())) ) ) {
                                ...
                                //volt prejobja, ha remote csatorna file-ja volt, azt masolja
                                ...
                                Statement stmt3 = conn.createStatement();
                                stmt3.executeUpdate(sql);
                                sql = "SELECT * FROM output_prop WHERE id_workflow='" + wfid + "' and name='remote' " +
                                        "and id_aoutput=(SELECT id FROM aoutput WHERE seq='"+op.getPreoutput()+"' and id_ajob=(SELECT id FROM ajob WHERE id_aworkflow='"+oldgrafid+"' AND name='"+op.getPrejob()+"') )";                                
                                ...
                                while (rst3.next()) {
                                    ...
                                    // System.out.println("-----------old remote output:" + remote);
                                }
                                rst3.close();
                                if (!"".equals(remote)) {
                                    sql = "INSERT INTO input_prop VALUES(" + wfid + "," + id_ainput+ ",'remote','" + remote + "','" + rlabel + "','" + rdesc + "','" + rinh + "')";
                                    // System.out.println("-----------copy to remote input:" + remote);
                                    stmt3.executeUpdate(sql);
                                }
                                stmt3.close();                                
                            }
...
//                            }
                        ...
                        if (oldinputs.size() > 0) {
                            // System.out.println("### delete oldinputs:" + oldinputs.keySet().toString());
                            Enumeration oinames = oldinputs.keys();
                            String oiids = "";//old input id
...
//output ports copy
                        ...
                        // System.out.println("### all oldoutputs:" + oldoutputs.keySet().toString());

                        //output ports of new graph (job) 
                        sql = "SELECT * FROM aoutput as ao WHERE ao.id_ajob='" + id_ajob + "' ";
                        rst2 = stmt2.executeQuery(sql);
                        while (rst2.next()) {//uj graf(job) output portjai - masolas
                        ...
                        if (oldoutputs.size() > 0) {
                            // System.out.println("### delete oldoutputs:" + oldoutputs.keySet().toString());
                            Enumeration oonames = oldoutputs.keys();
                            String oiids = "";//old output id
...
//delete remaining job props
            if (oldwf.size() > 0) {
                // System.out.println("#### ---- oldWF delete job_prop io_prop :" + oldwf.keySet().toString());
                Enumeration oonames = oldwf.keys();
                String oiids = "";//old WF job id
                while (oonames.hasMoreElements()) {
...
//wf graph chaginging
            ...
            conn.close();
            // System.out.println("************** milisec:" + (System.currentTimeMillis() - s));
        } catch (SQLException e) {
            e.printStackTrace();
            return false;
        } catch (Exception ee) {
    ...
    {
        Base.getI().deleteFromJobIdCache(value.getPortalID(),value.getUserID(),value.getWorkflowID());
//       System.out.println("WWWW wfs:setNewTemplate:"+value.getGraf()); 
       String newgraf="";
       try
        {
            Connection conn = Base.getConnection();
...
//            System.out.println("WWWW wfs:setNewTemplate:grafid"+templeteid); 
    ...
    public VolatileBean getVolatileOutputs(ComDataBean value) {
        ...
        try {
            // System.out.println("getVolatileOutputs(" + value.getWorkflowID() + ")...");
            Connection conn = Base.getConnection();
            //
            // get keys: workflowID, aworkflowID
            long workflowID = 0;
            long aworkflowID = 0;
            //
            String sql = "SELECT w.id, aw.id " +
                    "FROM aworkflow as aw, workflow as w " +
                    "WHERE w.name=? and w.id_aworkflow=aw.id " +
                    "and aw.id_portal=? and aw.id_user=?";
            ...
            // System.out.println("workflowID : " + workflowID + ", aworkflowID : " + aworkflowID);
            //
            // 
            // get job and output name from graph (aoutput) table and
            // output name from concrete workflow config (output_prop) table...
            //
            sql = "SELECT aj.name, ao.name, op.value " +
                    "FROM ajob as aj, aoutput as ao, output_prop as op " +
                    "WHERE aj.id_aworkflow=? and op.id_workflow=? " +
                    "and aj.id=ao.id_ajob and op.id_aoutput=ao.id " +
                    "and op.name='intname' " +
                    "and op.id_aoutput IN (SELECT op.id_aoutput " +
                    "FROM ajob as aj, aoutput as ao, output_prop as op " +
                    "WHERE aj.id_aworkflow=? " +
                    "and aj.id=ao.id_ajob and op.id_aoutput=ao.id " +
                    "and op.name='type' and op.value='volatile')";
            ...
            while (rst.next()) {
                ...
                // System.out.println("job and output names: " + jobName + ", " + outputName1 + ", " + outputName2);
